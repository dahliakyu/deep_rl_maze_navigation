...
class SarsaAgent:
    def __init__(self, env, action_size, alpha=0.1, gamma=0.9, epsilon=0.1):
        ...
        self.q_table = np.zeros((self.env_size, self.env_size, action_size))  # Q-table initialized to zeros. Dimensions: (rows, columns, actions)
    ...
    def update_q_value(self, state, action, reward, next_state, next_action):  # Added next_action
        x, y = state
        next_x, next_y = next_state
        q_next = self.q_table[next_x, next_y, next_action]  # Q-value of the next action
        self.q_table[x, y, action] += self.alpha * (reward + self.gamma * q_next - self.q_table[x, y, action])
